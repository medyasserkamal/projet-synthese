...
<div class="animate-fade-in" style="max-width: 600px; margin: 0 auto;">
    <div class="d-flex align-center mb-2" style="gap: 1rem;">
        ...
        <h1 style="margin: 0; font-family: 'Outfit', sans-serif;">Créer une Catégorie</h1>
    </div>

    <form action="{{ route('admin.categories.store') }}" method="POST" class="card" style="padding: 2rem; border-radius: var(--radius-lg);">
        @csrf
        
        <div class="mb-2">
            <label for="name" style="display: block; font-weight: 700; margin-bottom: 0.5rem;">Nom de la catégorie</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Technologie, Voyage, Cuisine..." required oninput="generateSlug(this.value)">
            @error('name')
        ...
        <div class="mb-2">
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="technologie" required>
            @error('slug')
                <p style="color: var(--accent-red); font-size: 0.8rem; margin-top: 0.5rem;">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-2">
            ...
            <textarea name="description" id="description" rows="5" placeholder="Articles sur les dernières innovations technologiques..." style="resize: vertical;">{{ old('description') }}</textarea>
            @error('description')
                <p style="color: var(--accent-red); font-size: 0.8rem; margin-top: 0.5rem;">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 1rem; margin-top: 1rem;">
            ...
            <button type="submit" class="btn">
                <i data-feather="check" style="width: 18px; height: 18px;"></i> Créer la catégorie
            </button>
        </div>
    </form>
</div>
